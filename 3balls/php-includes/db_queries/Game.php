...
class Game {
    ...
    public function totalBet(){
        $query = 'SELECT game_time, sum(amount) as total_bet
                  FROM bets WHERE CAST(date_placed AS DATE) = CURDATE() 
                  GROUP BY CAST(date_placed AS DATE), game_time';
    ...
    public function transHistory($username){
        $query = 'SELECT id, game_time, 1st_ball, 2nd_ball, 3rd_ball, status, CAST(date_placed AS DATE) as date FROM bets 
                  WHERE username = ?
                  ORDER BY id DESC'; 
        $stmt = $this->conn->prepare($query);
